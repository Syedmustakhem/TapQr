...
export const createReviewSchema = z.object({
  ...
  reviewerEmail: z.string().trim().toLowerCase().email("Invalid email address.").nullable().optional(),
  rating: z.number().int().min(1, "Rating must be between 1 and 5.").max(5, "Rating must be between 1 and 5."),
  title: optionalText(120, 2),
  comment: optionalText(2000, 2),
}).strict().superRefine((data, ctx) => {
  if (!data.reviewerName && !data.reviewerEmail) {
    ctx.addIssue({ code: "custom", path: ["reviewerName"], message: "Provide a reviewer name or email address." });
  }
});
