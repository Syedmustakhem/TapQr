...
export class ReviewsRepository {
  ...
  async create(data: CreateReviewData) {
    return prisma.review.create({
      data,
      select: { ...publicReviewSelect, reviewerEmail: true },
    });
  }
  ...
  async findPublishedByBusiness(businessId: string, page: number, limit: number) {
    const where: Prisma.ReviewWhereInput = { businessId, status: ReviewStatus.PUBLISHED };
    const skip = (page - 1) * limit;
    const [reviews, total] = await prisma.$transaction([
      prisma.review.findMany({
        where, select: publicReviewSelect,
  ...
  async findManagedByBusiness(
    businessId: string,
    options: { page: number; limit: number; status?: ReviewStatus; rating?: number; verified?: boolean; search?: string }
  ) {
    ...
    const [reviews, total] = await prisma.$transaction([
      prisma.review.findMany({
        ...
        orderBy: [{ createdAt: "desc" }, { id: "desc" }],
        skip, take: options.limit,
      }),
      prisma.review.count({ where }),
    ]);
    return { reviews, total, page: options.page, limit: options.limit, totalPages: Math.ceil(total / options.limit) };
  }

  async updateStatus(id: string, data: { status: ReviewStatus; moderationNote?: string | null }) {
    return prisma.review.update({
      where: { id },
      data: { status: data.status, moderationNote: data.moderationNote ?? null, moderatedAt: new Date() },
      select: { ...publicReviewSelect, reviewerEmail: true, moderationNote: true },
    });
  }
  ...
  async createReport(data: { reviewId: string; businessId: string; userId?: string | null; reason: string; details?: string | null }) {
    return prisma.reviewReport.create({
      data,
      select: { id: true, reviewId: true, businessId: true, userId: true, reason: true, details: true, createdAt: true },
    });
  }

  async hasUserReported(reviewId: string, userId: string) {
    return prisma.reviewReport.findFirst({ where: { reviewId, userId }, select: { id: true } });
  }
  ...
  async getSummary(businessId: string) {
    ...
    return {
      ...
      totalReviews,
      verifiedReviews: await prisma.review.count({
        where: { businessId, status: ReviewStatus.PUBLISHED, isVerified: true },
      }),
